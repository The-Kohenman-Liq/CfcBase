...
class CfcInter(CfcGroup):
    ...
    сказывается хорошо. Здесь реализовано через применение знаковой маски к абсолютному(на данный момент нет – abs стабильно вызывает взрыв градиентов) 
    значению проходящего сигнала. Детерменизм обеспечивается за счет generator.
    """
    generator: torch.Generator
    use_masks: bool = True
    ...
    # надо дергать извне после optimiser step. В CfcMemoryModel за это отвечает метод scheduler_step
    @override
...
# в рамках большинства задач концепт бессмысленный, как будто – в оригинале нужен был для управления дроном.
# но тем не менее – TODO! (может быть)
